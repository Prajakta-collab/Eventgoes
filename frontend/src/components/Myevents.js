...
const Myevents = () => {
  ...
  // const minTime=currentDate.getHours()+":"+currentDate.getMinutes();
    ...
    const [event, setevent] = useState({id:" ",etitle:"", edescription:"",etag:"default",edate:"",etime:"",evenue:""})
    ...
    const updateEvent=(CurrentEvent)=>{
        ref.current.click();
        setevent({id:CurrentEvent._id ,etitle:CurrentEvent.title, edescription:CurrentEvent.description, etag:CurrentEvent.tag,edate:CurrentEvent.date,etime:CurrentEvent.time,evenue:CurrentEvent.venue})
    }
    ...
    const handleClick=(e)=>{
        e.preventDefault();
        editEvent(event.id,event.etitle,event.edescription,event.etag,event.edate,event.etime,event.evenue)
    ...
    <div className="modal-content">
     ...
     >
      ...
      <div className="modal-footer">
        ...
        <button disabled={event.etitle.length<5 || event.edescription.length<5} type="button" className="btn btn-primary" onClick={handleClick}>Update Event</button>
      </div>
    </div>
  </div>
</div>
